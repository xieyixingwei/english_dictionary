...
class _WordDetailsState extends State<WordDetails> {
  ...
  List<Widget> _buildTags(BuildContext context) {
    ...
    List<Widget> tags = [];
    if(widget._word.w_tags == null || widget._word.w_tags.length == 0) return tags;
    tags.add(Text('Tags:', style: style));
    tags.addAll(
      widget._word.w_tags.map((e) => 
        Tag(
          label: Text(e, style: style,),
          onDeleted: widget._editable ? () => setState(() => widget._word.w_tags.remove(e)) : null,
        )
      ).toList()
    );
    return tags;
  ...
  List<Widget> _buildEtyma(BuildContext context) {
    ...
    List<Widget> etyma = [];
    if(widget._word.w_etyma == null || widget._word.w_etyma.length == 0) return etyma;
    etyma.add(Text('词根:', style: style));
    etyma.addAll(
      widget._word.w_etyma.map((e) => 
        Tag(
          label: Text(e, style: style,),
          onDeleted: widget._editable ? () => setState(() => widget._word.w_etyma.remove(e)) : null,
        )
      ).toList()
    );
    return etyma;
